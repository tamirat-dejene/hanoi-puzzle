...
const TowerOfHanoi: React.FC = () => {
  ...
  const [canvasRef, moves, movesLog, handleSolve, solved, solving, handleRestart] = useCanvas(diskCount);
      ...
      <div className="header-container">
        <div className="controls-container">
          <div className="controls">
            <label>Disks:</label>
            <button className="control-button" onClick={() => setDiskCount((prev) => solving ? prev : Math.max(3, prev - 1))}>▼</button>
            <span className="disk-count">{diskCount}</span>
            <button className="control-button" onClick={() => setDiskCount((prev) => solving ? prev : Math.min(8, prev + 1))}>▲</button>
          </div>
          <div className="action-buttons">
            <button className="action-button" onClick={() => {if (!solving) handleRestart()}}>Restart</button>
